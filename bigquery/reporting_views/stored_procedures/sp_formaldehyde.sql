MERGE
 `reporting.aqs_formaldehyde` AS destination
USING
 (
 SELECT
   ...
   ingestion_timestamp
 FROM
   `warehouse.aqs_formaldehyde`
 WHERE
   ingestion_timestamp = ingestion_time_ts
   # Toxics Primary Sample POC Codes
   AND ((poc=2 AND CONCAT(state_code,county_code,site_number) NOT IN ('360810124','360050133') ) OR ( poc=4 AND CONCAT(state_code,county_code,site_number) IN ('360810124', '360050133') )
     #Toxics & Carbonyls Replicate Samples
     OR (poc=20)
     # Toxics & Carbonyls Duplicate Samples
     OR (poc=10)) ) AS source
ON

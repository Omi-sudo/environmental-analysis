MERGE
 `reporting.aqs_toxics_and_carbonyls` AS destination
USING
 (
 SELECT
   ...
   ingestion_timestamp
 FROM
   `warehouse.aqs_toxics_and_carbonyls`
 WHERE
   ingestion_timestamp = ingestion_time_ts
   AND ((poc=2 AND CONCAT(state_code,county_code,site_number) NOT IN ('360810124','360050133'))
      # for NYBG and Queens College 2
     OR (poc=4 AND CONCAT(state_code,county_code,site_number) IN ('360810124','360050133') )
      #  Toxics and carbonyls replicate samples
     OR (poc=20)
     #  Toxics and carbonyls duplicate samples
     OR (poc=10)
     OR (TRIM(UPPER(sample_duration)) = "1 HOUR" AND CAST(method_code AS INT64) = 128 AND TRIM(UPPER(sample_frequency)) = "HOURLY")
     OR (TRIM(UPPER(sample_duration)) = "1 HOUR" AND (poc IN (3,4)) AND TRIM(UPPER(method_type)) = "FEM" )) ) AS source
ON

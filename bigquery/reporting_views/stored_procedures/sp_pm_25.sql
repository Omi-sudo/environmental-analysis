MERGE
 `reporting.aqs_pm_25` AS destination
USING
 (
 SELECT
 ...
 WHERE
   ...
   # PM 2.5 (88101) FRM Collocated Samples PM 2.5 (88101) FEM
   AND ((poc IN (2,3,4) AND TRIM(UPPER(sample_frequency)) = "HOURLY" AND (TRIM(UPPER(method_type)) IN ("FEM","FRM")) AND CAST(parameter_code AS INT64) IN (88101))
   # PM 2.5 (88502) Acceptable PM2.5 AQI &amp; Speciation Mass
     OR (CAST(parameter_code AS INT64) IN (88502) AND TRIM(UPPER(parameter)) IN ("Acceptable PM2.5 AQI & Speciation Mass") AND TRIM(UPPER(method_type)) IN ("Non-FRM"))) ) AS source
ON

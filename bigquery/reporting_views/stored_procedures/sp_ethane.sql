MERGE
  `reporting.aqs_ethane` AS destination
USING
  (
  SELECT
...
ON
  ...
  AND destination.date_of_last_change < source.date_of_last_change
  WHEN MATCHED THEN UPDATE SET destination.cbsa_code = source.cbsa_code, destination.state_code=source.state_code, destination.county_code = source.county_code, destination.qualifier = source.qualifier, destination.uncertainty = source.uncertainty, destination.detection_limit = source.detection_limit, destination.units_of_measure=source.units_of_measure, destination.sample_duration = source.sample_duration, destination.sample_measurement = source.sample_measurement, destination.datum = source.datum, destination.latitude = source.latitude, destination.longitude=source.longitude, destination.sample_frequency = source.sample_frequency, destination.units_of_measure_code = source.units_of_measure_code, destination.parameter = source.parameter, destination.method = source.method, destination.method_code=source.method_code, destination.method_type=source.method_type, destination.date_gmt=source.date_gmt, destination.time_gmt=source.time_gmt, destination.date_of_last_change=source.date_of_last_change, destination.record_date=source.record_date, destination.ingestion_timestamp=source.ingestion_timestamp
  WHEN NOT MATCHED
  THEN
INSERT
  ( cbsa_code,
    ...
    ingestion_timestamp)
VALUES
  ( source.cbsa_code, source.state, source.state_code, source.county, source.county_code, source.site_number, source.qualifier, source.uncertainty, source.detection_limit, source.units_of_measure, source.sample_duration, source.sample_measurement, source.datum, source.latitude, source.longitude, source.sample_frequency, source.units_of_measure_code, source.poc, source.parameter_code, source.sample_duration_code, source.parameter, source.method, source.method_code, source.method_type, source.date_local, source.time_local, source.date_gmt, source.time_gmt, source.date_of_last_change, source.record_date, source.ingestion_timestamp);

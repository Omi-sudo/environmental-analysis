"""
arcgis_e2e_test.py
Python module for ArcGIS e2e test
"""
...
GCS_FILE_PATH = f"parsed/remediation_sites/{constant.CURRENT_DATE}/" \
                f"remediation_sites_1654856052.json"
BQ_TABLE_NAME = f"{constant.E2E_DATASET}." \
                f"arcgis_remediation_sites_e2e"
...
def delete_gcs_object():
  ...
  with subprocess.Popen(f"gsutil rm gs://{constant.E2E_BUCKET}/raw/"
                        f"remediation_sites/{constant.CURRENT_DATE}/"
                        "remediation_sites_1654856052.json",
...
def fetch_cloud_run_endpoint():
  ...
  for itr in data:
    if "https://arcgis-e2e-testing" in itr:
...
def delete_records_from_bq(bq_table_name, project_id):
  ...
  if "job_audit" in bq_table_name:
    ...
    batch_timestamp="2021-06-10 10:14:12.700268"
    """
  else:
...
def read_data_from_storage_and_ingest_to_bq():
  ...
  gcs_data = [
    dict(
      ...
      **{
        "ingestion_timestamp":
          datetime.strptime("2021-06-10 10:14:12.700268",

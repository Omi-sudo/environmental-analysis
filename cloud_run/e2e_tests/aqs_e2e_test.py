"""
aqs_e2e_test.py
Python module for AQS e2e test
"""
...
GCS_FILE_PATH = f"parsed/formaldehyde/{constant.CURRENT_DATE}/" \
                f"formaldehyde_1654856052.json"
BQ_TABLE_NAME = f"{constant.E2E_DATASET}.aqs_formaldehyde_e2e"
...
def delete_gcs_object():
  ...
  with subprocess.Popen(f"gsutil rm gs://{constant.E2E_BUCKET}/raw/"
                        f"formaldehyde/{constant.CURRENT_DATE}/"
                        "formaldehyde_1654856052.json",
...
def authenticate_and_trigger_cloud_run(url_info):
  ...
  parsed_cloud_run_url = url_info[0] + "/process_parse_layer?" \
                                       "layer_name=formaldehyde&" \
...
def fetch_cloud_run_endpoint():
  ...
  for itr in data:
    if "https://aqs-e2e-testing" in itr:
...
def delete_records_from_bq(bq_table_name, project_id):
  ...
  if "job_audit" in bq_table_name:
    ...
    batch_timestamp="2021-06-12 10:14:12.700268"
    """
  else:
...
def delete_cloud_run_service():
  ...
  with subprocess.Popen("gcloud run services delete "
                        "aqs-e2e-testing --region=us-east4 "
                        "--quiet", shell=True) as process:
    process.wait()
  logging.info("return code is %s", str(process.returncode))
  if process.returncode != 0:
    raise ValueError(
...
def read_data_from_storage_and_ingest_to_bq():
  ...
  gcs_data = [
    dict(
      ...
      **{
        "ingestion_timestamp":
          datetime.strptime("2021-06-12 10:14:12.700268",

"""
socrata_to_gcp.py Module
This is the main module for socrata apis.
...
socrata_raw_ingestion_list = []
socrata_raw_ingestion_tasks_name_list = []
socrata_parse_ingestion_list = []
socrata_parse_ingestion_tasks_name_list = []
socrata_raw_bq_ingestion_list = []
socrata_raw_bq_ingestion_tasks_name_list = []
socrata_raw_bq_job_audit_list = []
socrata_raw_bq_job_audit_tasks_name_list = []
socrata_wh_bq_ingestion_list = []
socrata_wh_bq_ingestion_tasks_name_list = []
socrata_wh_bq_job_audit_list = []
socrata_wh_bq_job_audit_tasks_name_list = []
socrata_reporting_bq_ingestion_list = []
socrata_reporting_bq_ingestion_tasks_name_list = []
socrata_reporting_bq_job_audit_list = []
socrata_reporting_bq_job_audit_tasks_name_list = []
...
try:
  ...
  with DAG("SOCRATA_TO_GCP",
           default_args=dag_default_args,
           description="DAG TO ORCHESTRATE SOCRATA API DATA TO GCP PIPELINE",
           max_active_runs=3,
           concurrency=30,
    ...
    parse_gcs_ingestion_starts = BranchPythonOperator(
      ...
      op_kwargs={"xcom_tasks_key_list": socrata_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": socrata_parse_ingestion_tasks_name_list,
    ...
    bq_wh_job_audit_ingestion_starts = BranchPythonOperator(
      ...
      python_callable=utils.get_xcoms_from_tasks,
      op_kwargs={"xcom_tasks_key_list": socrata_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": socrata_wh_bq_job_audit_tasks_name_list,
    ...
    for task_details in config.SOCRATA_TASKS_CONFIG:
      if task_details["ingestion_layer"] == "gcs_raw":
        if task_details["data_source"] == "socrata":
          socrata_raw_ingestion_list.append(
            PythonOperator(
              task_id=task_details["task_name"],
              python_callable=utils.authenticate_and_trigger_cloud_run,
              op_kwargs={
                "url": constants.SOCRATA_CLOUD_RUN_SERVICE_ENDPOINT,
          ...
          socrata_raw_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "gcs_parse":
        if task_details["data_source"] == "socrata":
          socrata_parse_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.authenticate_and_trigger_cloud_run,
            op_kwargs={
              "url": constants.SOCRATA_CLOUD_RUN_SERVICE_ENDPOINT,
          ...
          socrata_parse_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_raw":
        if task_details["data_source"] == "socrata":
          socrata_raw_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
          ...
          socrata_raw_bq_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_raw_job_audit":
        if task_details["data_source"] == "socrata":
          socrata_raw_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
          ...
          socrata_raw_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_warehouse":
        if task_details["data_source"] == "socrata":
          socrata_wh_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
          ...
          socrata_wh_bq_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_wh_job_audit":
        if task_details["data_source"] == "socrata":
          socrata_wh_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.bq_insert_job_audit_rows,
            op_kwargs={
              "parsed_gcs_file_path":
          ...
          socrata_wh_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_reporting":
        if task_details["data_source"] == "socrata":
          socrata_reporting_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.
              read_data_from_source_bq_and_ingest_destination_bq,
            op_kwargs={
        ...
        if task_details["data_source"] == "socrata":
          socrata_reporting_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.bq_insert_job_audit_rows,
            op_kwargs={
              "parsed_gcs_file_path":
          ...
          socrata_reporting_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])
        ...
        raw_gcs_ingestion_ends.set_upstream(socrata_raw_ingestion_list)
        ...
        parse_gcs_ingestion_ends.set_upstream(socrata_parse_ingestion_list)

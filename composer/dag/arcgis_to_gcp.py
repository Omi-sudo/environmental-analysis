"""
arcgis_to_gcp.py Module
This is the main module for arcgis_apis apis.
This module will trigger the respective
cloud run service and proceed
parsed data to gcp in ingestion,
...
arcgis_raw_ingestion_list = []
arcgis_raw_ingestion_tasks_name_list = []
arcgis_parse_ingestion_list = []
arcgis_parse_ingestion_tasks_name_list = []
arcgis_raw_bq_ingestion_list = []
arcgis_raw_bq_ingestion_tasks_name_list = []
arcgis_raw_bq_job_audit_list = []
arcgis_raw_bq_job_audit_tasks_name_list = []
arcgis_wh_bq_ingestion_list = []
arcgis_wh_bq_ingestion_tasks_name_list = []
arcgis_wh_bq_job_audit_list = []
arcgis_wh_bq_job_audit_tasks_name_list = []
arcgis_reporting_bq_ingestion_list = []
arcgis_reporting_bq_ingestion_tasks_name_list = []
arcgis_reporting_bq_job_audit_list = []
arcgis_reporting_bq_job_audit_tasks_name_list = []
...
try:
  ...
  with DAG("ArcGIS_TO_GCP",
           default_args=dag_default_args,
           description=
           "DAG TO ORCHESTRATE ArcGIS API DATA TO GCP PIPELINE",
           max_active_runs=3,
           concurrency=30,
    ...
    parse_gcs_ingestion_starts = BranchPythonOperator(
      ...
      op_kwargs={"xcom_tasks_key_list": arcgis_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": arcgis_parse_ingestion_tasks_name_list,
    ...
    bq_raw_ingestion_starts = BranchPythonOperator(
      ...
      python_callable=utils.get_xcoms_from_tasks,
      op_kwargs={"xcom_tasks_key_list": arcgis_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": arcgis_raw_bq_ingestion_tasks_name_list,
    ...
    bq_raw_job_audit_ingestion_starts = BranchPythonOperator(
      ...
      python_callable=utils.get_xcoms_from_tasks,
      op_kwargs={"xcom_tasks_key_list": arcgis_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": arcgis_raw_bq_job_audit_tasks_name_list,
    ...
    bq_wh_ingestion_starts = BranchPythonOperator(
      ...
      python_callable=utils.get_xcoms_from_tasks,
      op_kwargs={"xcom_tasks_key_list": arcgis_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": arcgis_wh_bq_ingestion_tasks_name_list,
    ...
    bq_wh_job_audit_ingestion_starts = BranchPythonOperator(
      ...
      python_callable=utils.get_xcoms_from_tasks,
      op_kwargs={"xcom_tasks_key_list": arcgis_raw_ingestion_tasks_name_list,
                 "upstream_tasks_list": arcgis_wh_bq_job_audit_tasks_name_list,
    ...
    for task_details in config.ARCGIS_TASKS_CONFIG:
      if task_details["ingestion_layer"] == "gcs_raw":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_raw_ingestion_list.append(
            PythonOperator(
              task_id=task_details["task_name"],
              python_callable=utils.authenticate_and_trigger_cloud_run,
              op_kwargs={
                "url": constants.ARCGIS_CLOUD_RUN_SERVICE_ENDPOINT,
          ...
          arcgis_raw_ingestion_tasks_name_list.append(task_details["task_name"])

      elif task_details["ingestion_layer"] == "gcs_parse":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_parse_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.authenticate_and_trigger_cloud_run,
            op_kwargs={
              "url": constants.ARCGIS_CLOUD_RUN_SERVICE_ENDPOINT,
          ...
          arcgis_parse_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_raw":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_raw_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
          ...
          arcgis_raw_bq_ingestion_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_raw_job_audit":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_raw_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
            ...
            op_kwargs={
              "parsed_gcs_file_path":
                task_details["parsed_gcs_file_path"],
              "bq_table_name": task_details["bq_table_name"],
              "query": task_details["query"] + "'{{ti.xcom_pull"
                                               "(task_ids="
                                               "'RAW_GCS_INGESTION_STARTS',"
          ...
          arcgis_raw_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])

      elif task_details["ingestion_layer"] == "bq_warehouse":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_wh_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
          ...
          arcgis_wh_bq_ingestion_tasks_name_list.append(
            task_details["task_name"])
      elif task_details["ingestion_layer"] == "bq_wh_job_audit":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_wh_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.bq_insert_job_audit_rows,
            op_kwargs={
              "parsed_gcs_file_path":
          ...
          arcgis_wh_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])


      elif task_details["ingestion_layer"] == "bq_reporting":
        if task_details["data_source"] == "arcgis_apis":
          arcgis_reporting_bq_ingestion_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.
              read_data_from_source_bq_and_ingest_destination_bq,
            op_kwargs={
        ...
        if task_details["data_source"] == "arcgis_apis":
          arcgis_reporting_bq_job_audit_list.append(PythonOperator(
            task_id=task_details["task_name"],
            python_callable=utils.bq_insert_job_audit_rows,
            op_kwargs={
              "parsed_gcs_file_path":
          ...
          arcgis_reporting_bq_job_audit_tasks_name_list.append(
            task_details["task_name"])
        ...
        raw_gcs_ingestion_ends.set_upstream(arcgis_raw_ingestion_list)
        ...
        parse_gcs_ingestion_ends.set_upstream(arcgis_parse_ingestion_list)
